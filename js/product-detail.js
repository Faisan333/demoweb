...
let appliedPromo = "Fasica123"; // GLOBAL STORAGE

/* TOGGLE PROMO */
function togglePromo() {
  const box = document.getElementById("promoContent");
...
document.getElementById("promoCode").addEventListener("input", function () {
    ...
    const promoInput = this.value.trim();
    const buyBtn = document.querySelector(".add-cart-btn");

    if (promoInput !== "" && promoInput !== appliedPromo) {

        buyBtn.disabled = true;
        buyBtn.setAttribute("onclick", "return false;"); // BLOCK CLICK

    } else {

        buyBtn.disabled = false;
        buyBtn.setAttribute("onclick", "buyNow()"); // RESTORE CLICK
    }
});
...
function buyNow() {
    ...
    // DEFAULT MESSAGE
    let message =
        "Hello, I would like to place an order for Face Oil.";

    // ADD PROMO ONLY IF FILLED
    if (promoValue) {
